...
// Capture exception captures an error.
func Capture(exception error) *EventID {
	hub := CurrentHub()
	return hub.CaptureException(exception)
}

// Capture exception captures an error with gin.Context
func CaptureWith(ctx *gin.Context, exception error) *EventID {
	if hub := getHubFromGinContext(ctx); hub != nil {
		return hub.CaptureException(exception)
	}
	hub := CurrentHub()
	return hub.CaptureException(exception)
...
// GetHubFromContext retrieves attached *Hub instance from gin.Context.
func getHubFromGinContext(ctx *gin.Context) *Hub {
	if hub, ok := ctx.Get("sentry"); ok {
		if hub, ok := hub.(*Hub); ok {
			return hub
		}
	}
	return nil
